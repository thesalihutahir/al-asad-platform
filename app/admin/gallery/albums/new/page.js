...
export default function CreateAlbumPage() {
    ...
    const checkDuplicateTitle = async (title) => {
        ...
        setIsChecking(true);
        try {
            const q = query(collection(db, "gallery_albums"), where("title", "==", title.trim()));
            const snapshot = await getDocs(q);

            if (!snapshot.empty) {
                setDuplicateWarning(`An album named "${title}" already exists.`);
            } else {
                setDuplicateWarning(null);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showSuccess({
                title: "Album Created!",
                message: "Your new photo album has been created successfully.",
...
return (
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto pb-12">
            ...
            <div className="flex items-center gap-4 py-4 border-b border-gray-200">
                ...
                <div>
                    <h1 className="font-agency text-3xl text-brand-brown-dark">Create New Album</h1>
                    <p className="font-lato text-sm text-gray-500">Organize photos into a collection.</p>
                </div>
            </div>
